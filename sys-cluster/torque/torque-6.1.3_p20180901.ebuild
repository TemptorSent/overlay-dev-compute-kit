...
inherit git-r3 flag-o-matic eutils autotools linux-info

DESCRIPTION="Resource manager and queuing system based on OpenPBS"
HOMEPAGE="http://www.adaptivecomputing.com/products/torque"

SRC_URI=""

MY_PL="${PV##*_p}"
MY_PV="${PV%_p*}"
[ "x${MY_PL}" = "x${PV}" ] && MY_PL="0"

EGIT_REPO_URI="https://github.com/adaptivecomputing/torque.git"
EGIT_BRANCH="${MY_PV}"

if [ ${MY_PL} -gt 19000101 ] ; then
                MYCY="${MY_PL%????}"
...
src_prepare() {
	...
	sed -e 's:/sbin/ldconfig;:/bin/true:/g' -i src/{resmom,daemon_client}/Makefile{,.am,.in}
	eapply_user
	_elibtoolize
	eautoreconf
}
...
src_configure() {
	...
	econf \
		...
		$(usex kernel_linux $(use_enable cpusets geometry-requests) --disable-geometry-requests) \
